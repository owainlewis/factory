...
func TestBuildRepoWorkflow(t *testing.T) {
	dir := t.TempDir()
	workflowDir := filepath.Join(dir, ".factory", "WORKFLOWS")
	...
	if !strings.HasSuffix(source, ".factory/WORKFLOWS/triage.md") {
		t.Fatalf("source = %q", source)
	}
...
func TestBuildRepoWorkflowFallsBackToRootFiles(t *testing.T) {
	dir := t.TempDir()
