...
`DeleteExpired` accepts a small candidate set plus maximum rows and logical bytes for one transaction. Both bounds are validated before work starts: the row bound is at least one and the logical-byte bound is at least `MaxEventBytes` (64 KiB), so every valid stored event can make forward progress. Rows are considered in sequence order. Once at least one row is selected, the implementation stops before the next row would exceed either bound; it never exceeds a bound and never strands a valid maximum-sized event behind an unusably small configuration. It locks/rechecks each candidate, skips attempts that became ineligible, deletes the selected range, and commits. Partial cleanup is expected: later sweeps continue from remaining rows, and an attempt stays eligible until all its event rows are gone. Repeating a batch after a crash is safe and counts only rows actually removed.
...
12. **Bounded deletion:** zero row bounds and byte bounds below 64 KiB are rejected; row and byte bounds are honored; a maximum-sized next event is deleted when the minimum valid bounds are used; eligibility is rechecked; partial and repeated sweeps converge; restart is safe; and unrelated events/history remain.
