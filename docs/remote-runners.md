# Run a Runner on a remote VM

Factory keeps its browser and operator API on loopback. Remote VMs connect to a
separate HTTPS listener that exposes only enrollment and the Runner lifecycle.
...
runner_listen = "0.0.0.0:7443"
runner_tls_cert = "/etc/factory/tls/server.crt"
runner_tls_key = "/etc/factory/tls/server.key"
```

All three `runner_*` settings are required together. The local browser API
remains available only at `127.0.0.1:7337`. Permit inbound TCP traffic to the
Runner port only from trusted Runner networks.

## Create a one-time enrollment

First print the stable identity on the VM without starting the Runner:
...
curl --fail-with-body \
  ...
  -d '{"worker_id":"PASTE_RUNNER_ID"}' \
  http://127.0.0.1:7337/api/v1/runner-enrollments
```
...
saves the returned Runner credential as `runner-credential` inside its
...
the credential when intentionally moving a Runner identity between servers.

## Connection behavior

Remote and local Runners use the same Jobs, claims, leases, cancellation, event,
and completion contract. Labels, capacity, and coding-agent capabilities appear
on the Runner profile. A disconnected VM becomes offline after its registration
...
authenticated Runner identity. Factory does not provision VMs, distribute
