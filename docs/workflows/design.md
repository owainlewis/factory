...
Factory can delegate a free-text task to a worker, but every operator must
...
habits. Factory will add reusable workflows: named, versioned prompts stored by the
...
unique lookup key. Other UTF-8 bytes remain unchanged; Factory does not perform
...
Factory is still a local, unauthenticated control plane bound to loopback. Any local
